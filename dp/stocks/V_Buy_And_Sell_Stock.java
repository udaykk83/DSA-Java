...
public class V_Buy_And_Sell_Stock {

	public static void main(String[] args) {
		int[] prices = { 1, 2, 3, 0, 2 };
		int n = prices.length;
		// only point to note here is dp array by 2
		...
		for (int idx = n - 1; idx >= 0; idx--) {
			for (int j = 0; j <= 1; j++) {
				...
				if (j == 1) {
					...
					int sellNow = +prices[idx] + dp[idx + 2][1];
	...
	public static int getMaxProfit(int[] prices, int idx, int tobuy, int dp[][]) {
		...
		if (tobuy == 1) {
			...
			int sellNow = +prices[idx] + getMaxProfit(prices, idx + 2, 1, dp);
